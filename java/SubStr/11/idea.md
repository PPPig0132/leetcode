...
优化1：当移除滑动窗口的不是最大值时，可以只比较新加入的和当前最大值
...
因此我们可以使用一个队列存储所有还没有被移除的下标。在队列中，这些**下标**按照**从小到大的顺序**被存储，并且它们在数组 nums 中对应的值是**严格单调递减**的。因为如果队列中有两个相邻的下标，它们对应的值相等或者递增，那么令前者为 i，后者为 j，就对应了上面所说的情况，即 nums[i] 会被移除，这就产生了矛盾。
